...
static void cs_rel(node_t me, int new_node_id) {
    ...
    if (state.active != 'u' && me->cs_new_id == new_node_id) {
    ...
    display_sc(me, 'V');
    XBT_VERB("Node %d: [%s:%d] '%c'/%d end of cs_rel()",
            me->self.id,
            __FUNCTION__,
            __LINE__,
            state.active,
...
static void rec_async_answer(node_t me, int idx, ans_data_t ans) {
    ...
    xbt_assert((*elem_ptr)->answer_data == NULL,
            "Node %d: [%s:%d] rec answer_data %d not NULL !",
            me->self.id,
            __FUNCTION__,
            __LINE__,
            idx);
    ...
    if ((*elem_ptr)->answer_data == NULL) {
        ...
        (*elem_ptr)->answer_data->answer.handle.dyn_ans_data =
            ans->answer.handle.dyn_ans_data;
    }
    ...
    if (req->type == TASK_CNX_REQ ||
        ...
        if (val_ret == OK) {
            ...
            if (strcmp(proc_data->proc_mailbox, req->answer_to) != 0) {
                ...
                answer.handle.dyn_ans_data = NULL;
...
static e_val_ret_t wait_for_completion(node_t me, int ans_cpt, int new_node_id, xbt_dynar_t dyn_ans_data) {
    ...
    XBT_IN("\tNode %d: [%s:%d] waiting for %d answers - dyn_ans_data = %p",
            me->self.id,
            __FUNCTION__,
            __LINE__,
            ans_cpt,
            ...
            if (!strcmp(MSG_task_get_name(task_received), "ans")) {
                ...
                if (dynar_idx != -1) {
                    ...
                    if (dynar_idx <= dynar_size - 1 && dynar_idx >= dynar_size - ans_cpt) {
                        ...
                        // fetch possible answers data
                        if (ans->type == TASK_BROADCAST && ans->br_type == TASK_SEARCH) {
                            if (ans->answer.handle.dyn_ans_data != NULL) {

                                xbt_assert(dyn_ans_data != NULL,
                                        "Node %d: [%s:%d] received '%s_%s' from %d - got answer data but dynar is uninitialized !",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        debug_msg[ans->type],
    ...
    *cpy_req_data = *req_data;
    ...
    while (res != MSG_TIMEOUT || cpy_req_data->type != TASK_GET_REP) {
        ...
        if (res != MSG_OK) {
            ...
            // reception failure
            XBT_INFO("Node %d: [%s:%d] Failed to receive the answer to my '%s' request from %s - result : %s",
                    cpy_req_data->sender_id,
                    __FUNCTION__,
                    __LINE__,
        ...
        /* NOTE : task_received n'est pas null ici si launch_fork_process l'a utilisé.
        xbt_assert(ans == NULL && task_received == NULL,
                "Node %d: [%s:%d] ans and task_received should"
...
static msg_error_t send_msg_async(node_t me,
    ...
    } while (res == MSG_TIMEOUT && loop_cpt < max_loops);    //TODO : task_sent et req_data sont perdus à chaque boucle, il faut les refaire
...
static e_val_ret_t broadcast(node_t me, u_req_args_t args, xbt_dynar_t dyn_ans_data) {
    ...
    switch(args.broadcast.type) {
        ...
        case TASK_SET_UPDATE:
            ret = set_update(me, args.broadcast.args->set_update.new_node_id,
                    args.broadcast.args->set_update.new_node_prio);
            break;
    ...
    // continue if set_update succeeded
    if (ret != UPDATE_NOK) {
    ...
    if (ret != UPDATE_NOK) {
        ...
        XBT_VERB("Node %d: [%s:%d] STOP BROADCAST",
                me->self.id,
                __FUNCTION__,
                __LINE__);
...
static void send_completed(node_t me, e_task_type_t type, int recipient_id, char* recipient_mailbox, int new_node_id) {
    ...
    XBT_DEBUG("Node %d: [%s:%d] '%s Completed' message sent to %d",
            me->self.id,
            __FUNCTION__,
            __LINE__,
            debug_msg[type],
...
static void init(node_t me) {
    ...
    XBT_INFO("Node %d: [%s:%d] init() ...", me->self.id, __FUNCTION__, __LINE__);
    int i, j;
...
/**
 * \brief Broadcast a search request among the whole DST
 * \param me the current node
...
static void broadcast_search(node_t me, int source_id, const char *item) {
    ...
    args.broadcast.first_call = 1;
    args.broadcast.source_id = me->self.id;
    args.broadcast.new_node_id = source_id;
    args.broadcast.lead_br = 0;

    args.broadcast.args = xbt_new0(u_req_args_t, 1);
    args.broadcast.args->search.source_id = source_id;
...
int node(int argc, char *argv[]) {
    ...
    if (join_success) {
        ...
        XBT_INFO("Node %d: **** JOIN COMPLETED **** - nb_ins_nodes = %d - order = %d",
                node.self.id,
                nb_ins_nodes,
                node.dst_infos.order);
        ...
        if (iter < 0) iter = 0;
        XBT_DEBUG("Node %d: last ten inserted nodes :", node.self.id);
        for (; iter < nb_ins_nodes; iter++) {
            XBT_DEBUG("\tinserted_nodes[%d] = %d", iter, inserted_nodes[iter]);
        }
 ...
 * \param dyn_ans_data pointer to a dynar to store possible answer data
    ...
    switch (type) {
        ...
        case TASK_BROADCAST:
            if (
                      ...
                      rcv_args.broadcast.type != TASK_REMOVE_STATE
                        ...
                        if (rcv_args.broadcast.type == TASK_SET_ACTIVE ||
                            ...
                            if (rcv_req->sender_id != me->self.id) {
                                ...
                                answer.handle.dyn_ans_data = (dyn_ans_data == NULL ? NULL : *dyn_ans_data);
                                ...
                                XBT_VERB("Node %d: [%s:%d] answer '%s' sent to %d",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                            ...
                            /* ... either store task */
                            XBT_VERB("Node %d: [%s:%d] - active = '%c' - new_id = %d:"
                    ...
                    } else {    /* -- ACCEPTED BROADCASTED TASKS -- */
                        ...
                        if (rcv_args.broadcast.first_call == 1) {
                            ...
                            if (rcv_args.broadcast.type == TASK_SET_UPDATE) {
                                ...
                                XBT_VERB("Node %d: [%s:%d] Run broadcast of Set Update for new_id = %d - state.new_id = %d",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        rcv_args.broadcast.args->set_update.new_node_id,
                            ...
                            XBT_VERB("Node %d: [%s:%d] broadcast first call - stage = %d -"
                            ...
                            if (me->self.id ==
                                ...
                                // I am the leader
                                XBT_DEBUG("Node %d: [%s:%d] Broadcast '%s' start",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        debug_msg[rcv_args.broadcast.type]);
                                ...
                                if (rcv_args.broadcast.type == TASK_SEARCH) {
                                    if (dyn_ans_data == NULL) {
                                        ...
                                        xbt_dynar_t loc_dyn_ans_data = xbt_dynar_new(sizeof(pu_ans_data_t), &xbt_free_ref);
                                        dyn_ans_data = &loc_dyn_ans_data;
                                    } else {
                                        if (*dyn_ans_data == NULL) {
                                ...
                                if (dyn_ans_data == NULL || *dyn_ans_data == NULL) {
                                ...
                                if (rcv_args.broadcast.type == TASK_SEARCH) {
                                    ...
                                    if (dyn_ans_data != NULL) {
                                        ...
                                        *dyn_ans_data = answer_data->answer.handle.dyn_ans_data;
                                    }
                                }
                            ...
                            if (rcv_args.broadcast.stage > 0) {
                                ...
                                XBT_VERB("Node %d: [%s:%d] Received Broadcast '%s' - lead_br = %d",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        debug_msg[rcv_args.broadcast.type],
                                ...
                                XBT_VERB("Node %d: [%s:%d] stage 0 - run broadcasted task - '%s'",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        debug_msg[rcv_args.broadcast.type]);

                                XBT_DEBUG("Node %d: [%s:%d] active = '%c' - new_id = %d",
                                        me->self.id,
                                        __FUNCTION__,
                                        __LINE__,
                                        state.active,
        ...
        */
        ...
        case TASK_SEARCH:
            ...
            xbt_assert(dyn_ans_data != NULL,
                    "Node %d: [%s:%d] dyn_ans_data shouldn't be NULL here !",
                    me->self.id,
                    __FUNCTION__,
                    __LINE__);
            ...
            if (*dyn_ans_data == NULL) {
                ...
                // NOTE : faut-il libérer ptr_u_ans_data ici ?
            }
        ...
        case TASK_BROADCAST_SEARCH:
            ...
            XBT_VERB("Node %d: TASK_BROADCAST_SEARCH done", me->self.id);
            break;
    }
