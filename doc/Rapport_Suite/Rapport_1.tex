...
\chapter{Foisonnement des messages : solution proposée}
...
attendues dans le cas de diffusions croisées.\footnote{Pour mémoire, les diffusions ont besoin de synchronisation à chaque étage, et utilisent donc aussi des accusés réception.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item il héberge ses propres files {\ti async\_answers} et {\ti sync\_answers} (les réponses asynchrones et
          synchrones attendues aux requêtes qu'il a émises)\footnote{C'est pour ce process qu'on maintient le mécanisme permettant de gérer les messages non attendus. Les autres n'en ont plus besoin, comme expliqué \hyperlink{ss_proc}{plus haut}.}
      \end{itemize}
...
\hypertarget{Failed}{}Il y a deux sortes d'échec d'insertion possibles.
...
peut avoir changé entre temps. Le leader doit donc dépiler la requête après exécution alors que le contact doit la
...
L'exécution d'une requête de la file est confiée à \hyperlink{LaunchForkProcess}{{\tt launch\_fork\_process()}}
...
\begin{itemize}
    ...
    \item {\ti buf\_new\_node\_id} : {\tt task.args.new\_node\_id} est mémorisé dans cette variable parce qu'on en a besoin
                                     après la destruction de {\ti task}.
\end{itemize}\hfill \\
On ôte la tâche courante de la file dans les cas suivants :
\begin{itemize}
	\item l'exécution a réussi ({\ti OK} ou {\ti UPDATE\_OK})
    ...
    \item l'exécution a échoué mais le n\oe ud courant n'est pas le contact direct du nouveau n\oe ud.
          ...
          Il s'agit du cas \hyperlink{Failed}{{\ti FAILED}} décrit plus haut.
\end{itemize}\hfill \\
%*********************************************************************
...
\begin{description}
	...
	L'émetteur empile alors la requête sur {\ti async\_answers} puis, lorsqu'un groupe de requêtes a été émis,
	...
	des accusés réception ait été reçu. \footnote{voir détails de \hyperlink{WaitForCompletion}{\tt wait\_for\_completion()} plus loin}
...
\chapter{Échec d'une diffusion d'un verrou : solution proposée}
\hypertarget{CsReq}{}
...
d'entrée en section critique}\\
	...
	capables de revenir à leur état initial en cas d'échec, sans nouvelle diffusion. Ces variables
	ainsi positionnées permettent de n'autoriser que le {\tt SET\_UPDATE} correspondant.
...
\begin{description}
	...
	\item [{\ti cs\_req\_time}:] Heure (locale) à laquelle la demande a été reçue
...
Pour le déterminer, j'ai réalisé les mesures présentées dans le tableau \ref{tab-cpt}, page \pageref{tab-cpt}.
\begin{table}[ht]
	%\centering
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
		\cline{3-10}
		\multicolumn{2}{c|}{}& \multicolumn{4}{c|}{1 000 n\oe uds} & \multicolumn{4}{c|}{4 000 n\oe uds} \\
		\cline{3-10}
		\multicolumn{2}{c|}{}& 1 & 2 & 3 & 4 & 1 & 2 & 3 & 4 \\
		\hline
		\multirow{2}{*}{BR\_CS\_REQ} & Échecs & 6 989 & 6 417 & 8 653 & 7 424 & 46 926 & 107 691 & 86 602 & 44 208\\
		\cline{2-10}                 & Succès &   273 &   275  & 277 & 272 & 1 040 & 1 080 & 1 072 & 1 100\\
		\hline
		\multirow{2}{*}{BR\_SET\_UPDATE} & Échecs & 49 & 52 & 57 & 51 & 174 & 264 & 235 & 231 \\
		\cline{2-10}                     & Succès & 224 & 223 & 220 & 221 & 866 & 816 & 837 & 869\\
		\hline
		\multicolumn{2}{|c|}{BR\_REMOVE\_STATE} & 49 & 52 & 57 & 51 & 174 & 264 & 235 & 231 \\
		\hline
		\multicolumn{2}{|c|}{Nouveau Contact} & 92 & 61 & 147 & 111 & 936 & 3 427 & 2 560 & 727 \\ 
		\hline
	\end{tabular}
  \vspace{0.5cm}\\
	\begin{tabular}{|c|c|c|c|c|c|}
  	\cline{3-6}
		\multicolumn{2}{c|}{}& \multicolumn{4}{c|}{10 000 n\oe uds}\\
		\cline{3-6}
		\multicolumn{2}{c|}{}& 1 & 2 & 3 & 4 \\
		\hline
		\multirow{2}{*}{BR\_CS\_REQ} & Échecs & 292 025 & 343 798 & 355 411 & 408 798\\
		\cline{2-6}                 & Succès & 2 481 & 2 668 & 2 694 & 2 559\\
		\hline
		\multirow{2}{*}{BR\_SET\_UPDATE} & Échecs & 508 & 608 & 625 & 550\\
		\cline{2-6}                     & Succès & 1 973 & 2 060 & 2 069 & 2009\\
		\hline
		\multicolumn{2}{|c|}{BR\_REMOVE\_STATE} & 508 & 608 & 625 & 550\\
		\hline
		\multicolumn{2}{|c|}{Nouveau Contact} & 12 063 & 11 363 & 15 771 & 13 543\\ 
		\hline
	\end{tabular}
...
\begin{itemize}
	...
	\item inclure les algorithmes de départ de n\oe uds (testés et fonctionnels en séquentiel) dans ces mécanismes. 
	\item faire des statistiques pour analyser plus précisément ce qu'il se passe pendant la construction (par exemple, nombre d'échecs de {\tt GET\_REP}, nombre de messages moyen par n\oe ud pour rejoindre le DST, etc ...)
	\item étudier la possibilité, en fin d'exécution de la simulation, de retourner l'ensemble des tables de routage sous la forme d'un fichier (XML, par exemple)
	qui décrirait notre DST résultant et qui pourrait servir de source pour un autre simulateur qui cette fois, simulerait un DST en fonctionnement, et pas
