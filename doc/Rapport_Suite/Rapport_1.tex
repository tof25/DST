...
\section{Proposition de solutions}

\paragraph {Solution au premier point,}principes de base
...
Le schéma général de cette solution à process multiples est présenté en figure \ref{call_seq}.
...
\begin{enumerate}
    ...
    \begin{itemize}
    	...
    	\item il héberge les files {\ti tasks\_queue} (les {\tt CNX\_REQ} en attente) et {\ti delayed\_tasks} (les tâches
          différées)
        \item
        \begin{inparaenum}[\itshape a\upshape)]
    ...
    \item {\tt Handle\_Task}
      ...
      C'est ce process qui est éventuellement créé par {\tt launch\_fork\_process()} pour traiter les
      requêtes concernées.
      Il héberge aussi ses propres files {\ti async\_answers} et {\ti sync\_answers}.
\end{enumerate}
...
\paragraph {run\_tasks\_queue()}\hfill \\
...
contact \rightarrow leader$. En cas d'échec, le contact doit refaire une tentive plus tard, mais son leader
...
\newgeometry{margin=.5cm}
  \begin{figure}
    \centering
    \includegraphics[height=\textheight]{./Images/Algo_run_tasks_queue.png}
  \end{figure}
\restoregeometry
...
{\ti nouveau n\oe ud}: le leader reçoit {\ti FAILED}; il dépile sa requête et répond {\ti UPDATE\_NOK} à son contact.
Celui-ci ne dépile rien et ne répond pas à nouveau n\oe ud. La requête restant dans la file s'exécutera
...
effet, pendant le temps d'exécution de {\tt CNX\_REQ}, d'autres demandes d'insertion ont pu arriver dans la
...
%
%On distingue deux cas : soit le n\oe ud courant est à l'état 'u', soit il est à 'a'.
%
%1) état 'u'
%-----------
%Variables:
...
%c'est à dire les CNX_GROUPS pour le même nouveau n\oe ud que celui en cours d'insertion.
%(c'est le test task_args.new_{\ti Node}_id == state.new_{\ti Node}_id)
%
...
%2) état 'a'
%-----------
%Ici, le n\oe ud courant est prêt à exécuter n'importe quelle autre tâche et on peut passer au reste de
%la file.
%
%Variables:
...
%
%    - is_contact :      vaut 1 si le n\oe ud courant est le contact direct du nouveau n\oe ud.
%                        (autrement dit, si l'émetteur de la tâche à exécuter est le nouveau n\oe ud)
%
%    - buf_new_{\ti Node}_id : task_args.new_{\ti Node}_id est mémorisé dans cette variable parce qu'on en a besoin
...
%        - l'exécution a échoué mais le n\oe ud courant n'est pas le contact direct du nouveau n\oe ud.
%          ({\ti UPDATE\_NOK} && !is_contact)
...
%*********************************************************************
%
%A noter : si le n\oe ud courant était verrouillé pour le même nouveau n\oe ud que
...
%  - xxx est l'id du n\oe ud courant
%  - nom_process est le nom attribué par cette fonction (tel que "Cnx_req" ou "Br_split", etc)
%  - yyy est l'id du nouveau n\oe ud en cours d'insertion
...
%- NEW_BROTHER_RCV ( fonction {\tt connection\_request()} )
...
%
%La fonction send_msg_sync() est chargée d'envoyer une requête à un autre n\oe ud, puis d'en attendre
%la réponse.
%
%Introduction : Chaque process hébergé par un n\oe ud possède deux files : sync_answers et async_answers.
%**************
