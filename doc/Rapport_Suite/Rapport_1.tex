...
Voir figure \ref{fig:rtq} en page \pageref{fig:rtq}.
...
Voir figures \ref{del-1} et \ref{del-2} en pages  \pageref{del-1} et \pageref{del-2}.
...
\paragraph {état '{\tt u}'} (voir partie 1 - figure \ref{del-1} en page \pageref{del-1})
...
\paragraph {état 'a'} (voir partie 2 - figure \ref{del-2} en page \pageref{del-2})
...
\subsection{envoi synchrone - {\tt send\_msg\_sync()}}
...
\newgeometry{top=.5cm, height=26cm}
\begin{figure}[ht]
    \centering
    \includegraphics[scale=.43]{./Images/Algo_send_msg_sync-1.png}
...
\begin{landscape}
  \begin{figure}[h]
    \centering
    \includegraphics[height=.91\textheight]{./Images/Algo_send_msg_sync-2.png}
...
\begin{description}
 ...
 \item [Requête:]
  Si c'est une demande de connexion ({\tt CNX\_REQ}), elle est simplement stockée dans {\ti tasks\_queue}. \footnote{On rappelle que cette file est hébergée par le n\oe ud courant.}
...
\item {\tt send\_msg\_sync()}
est la seule fonction susceptible d'empiler une requête dans {\ti sync\_answers}
...
\subsection{envoi asynchrone - {\tt send\_msg\_async()}}
