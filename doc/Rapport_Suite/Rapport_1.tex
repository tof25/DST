...
cette diffusion doit s'arrêter, mais il faut ôter les verrous qu'elle a déjà
...
\paragraph{Principes de base}\hfill

L'idée générale de cette nouvelle proposition est de procéder en deux temps:
\begin{enumerate}
	\item diffuser une première requête que j'appelle {\tt CS\_REQ}\footnote{Demande
d'entrée en section critique}\\
	En cas d'échec, cette diffusion ne posant pas de verrou mais positionnant des variables qui peuvent
éventuellement revenir à leur état antérieur sans nouvelle diffusion.

	\item si cette diffusion s'est bien passée, diffuser la pose de verrou proprement
dit -- requête {\tt SET\_UPDATE}
\end{enumerate}

\paragraph{Fonction cs\_req()}\hfill
