...
%TODO : parler du mécanisme de CS_REQ
...
\begin{enumerate}
    ...
    \begin{itemize}
    	...
    	\item il héberge les files {\ti tasks\_queue} (les tâches {\tt CNX\_REQ} en attente) et {\ti delayed\_tasks} (les tâches
          différées)
        \item
        \begin{inparaenum}[\itshape a\upshape)]
...
\paragraph {run\_tasks\_queue()}\label{rtq}\hfill \\
...
\paragraph {run\_delayed\_tasks()}\hfill \\
...
\subparagraph {état '{\tt u}'} (voir partie 1 - figure \ref{del-1})
...
\subparagraph {état 'a'} (voir partie 2 - figure \ref{del-2})
