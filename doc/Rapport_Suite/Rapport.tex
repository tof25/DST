...
1) état 'u'
-----------
Dans ce cas, un nouveau noeud est en cours d'insertion et les seules tâches exécutables ici sont
celles qui pourraient permettre de terminer cette insertion (toujours dans l'idée d'éviter les blocages)
Il s'agit des tâches CNX_GROUPS pour le même nouveau noeud que celui en cours d'insertion.
(c'est le test cnx_groups.new_node_id = state.new_node_id)

On parcourt donc la file à la recherche de ces tâches qu'on tente d'exécuter. (handle_task(task))

Lorsqu'on en trouve une, on l'ôte de la file sans s'assurer que son exécution a réussi ou pas
