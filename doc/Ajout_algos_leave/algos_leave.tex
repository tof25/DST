...
$pos\_me$ et $pos\_contact$ sont les positions des représentants des deux groupes qui ont fusionnés. On choisit de supprimer la position $pos\_me$.
\todo{préciser avec l'appel de la fonction}

S'il se trouve que c'est la position du n\oe ud courant (qu'on ne peut donc pas détruire), alors on échange les n\oe uds se trouvant aux positions $pos\_me$ et $pos\_contact$.
...
\begin{algorithmic}[1]
\Statex
\Procedure{replace\_bro}{$stage,\ init\_idx,\ new\_id$}	\Comment{$new\_id$ figure déjà à l'étage $stage$}
	\If{$(me.brothers[stage][init\_idx].id = new\_id\ ||\ {\tt index\_bro}(stage,\ new\_id) > -1)$}
		\State \Return
	\EndIf
	\Statex
	...
	\If{$(bro\_id \neq me.id \And init\_idx < {\tt size}(me.brothers[stage]))$}
		\State \Call{send\_msg\_async}{$bro\_id,\ {\tt del\_pred}(stage,\ me.id)$}
	\EndIf
	\Statex
	
	\If{$(new\_id \neq me.id)$}
		\State \Call{send\_msg\_async}{$bro\_id,\ {\tt add\_pred}(stage,\ me.id)$}
	\EndIf
