...
\begin{algorithm}
\caption{: fusionne des n\oe uds ``orphelins'' (\emph{source}) au groupe courant (\emph{cible})}	\label{a:mer}
\begin{algorithmic}[1]
\Statex
...
Mais dans le cas d'une diffusion (lignes \ref{a:mer1} à \ref{a:mer2}), $loc\_right$ ne reprend les mêmes valeurs que $right$ que si on ne se trouve pas sur la même extrémité de l'étage. Sinon, on change de côté. \todo{(à préciser)}
